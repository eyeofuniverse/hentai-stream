...
export default async function AdminHome() {
  ...
  const attention = [
    ...
    { label: "Dead video sources", value: deadSources, href: "/admin/series?sources=dead" },
  ].filter((a) => a.value > 0);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        <Stat label="Series" value={series} href="/admin/series" />
        <Stat label="Published" value={published} href="/admin/series?publish=PUBLISHED" tone="good" />
        <Stat label="Draft (metadata only)" value={draft} href="/admin/series?publish=DRAFT" />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card className="p-4">
          <SectionTitle right={<Link href="/admin/metadata" className="text-xs text-white/40 hover:text-white">details →</Link>}>
            Metadata sync
          </SectionTitle>
          {sync ? (
            ...
            <ul className="space-y-1 text-sm">
              {recentSources.map((s) => (
                <li key={s.id} className="flex items-center gap-2 text-white/70">
                  ...
                  <Link
                    href={`/admin/series/${s.episode.seriesId}`}
