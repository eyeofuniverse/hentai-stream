...
type Row = Awaited<ReturnType<typeof myWatchlist>>[number];
...
export default async function WatchlistPage({
  ...
  const { status = "ALL", sort = "added" } = await searchParams;
  ...
  if (sort === "title") {
    shown.sort((a, b) => a.series.title.localeCompare(b.series.title));
  } else if (sort === "rating") {
    const score = (r: Row) =>
      r.series.ratingCount > 0 ? r.series.ratingAvg : r.series.externalScore ?? 0;
    shown.sort((a, b) => score(b) - score(a));
  }
  // "added" = the query's default updatedAt desc order

  const qs = (patch: Record<string, string>) => {
    const p = new URLSearchParams();
    const merged = { status, sort, ...patch };
    for (const [k, v] of Object.entries(merged)) if (v && v !== "ALL" && v !== "added") p.set(k, v);
    const s = p.toString();
  ...
  return (
    <main className="mx-auto max-w-content px-4 py-8 lg:px-8">
      ...
      <h1 className="font-display text-2xl font-extrabold tracking-tight">My watchlist</h1>
      <p className="mt-1 text-sm text-white/45">
        {rows.length} series saved · tap the tag on a card to re-file or remove it
      </p>
          ...
          {SORTS.map((s) => (
            <Link
              ...
              className={`rounded px-2 py-1 transition ${
                sort === s.key ? "bg-white/10 text-white" : "text-white/45 hover:text-white"
              }`}
            >
              {s.label}
            </Link>
        ...
        <div className="mt-8 rounded-2xl border border-dashed border-line px-6 py-16 text-center">
          <p className="text-sm text-white/45">
            {status === "ALL"
              ...
              : `Nothing marked “${TABS.find((t) => t.key === status)?.label}”.`}
          </p>
          <Link
            href="/browse"
